"""
An example code to show how to train a Product VQ module on a Normal distribution using 4 codebooks.
"""
import torch
import torch.nn.functional as F
import torch.optim as optim
import numpy as np
from product_vq import PVQ
import os
import pickle
...
# Hyper-parameters
num_stages = 4 # Number of codebooks to apply Product VQ
vq_bitrate_per_stage = 7
embedding_dim = 128
learning_rate = 1e-3
batch_size = 64
...
vector_quantizer = PVQ(num_stages, vq_bitrate_per_stage, embedding_dim, device=device)
...
with open(f"used_codebooks_{num_stages}stages_{vq_bitrate_per_stage}bits_bs{batch_size}_lr{learning_rate}", "wb") as fp:
...
######################## Evaluation (Inference) of Product VQ #############################
